...
public class AI : MonoBehaviour
{
    ...
    void FixedUpdate()
    {
        ...
        if (gm.play)
        {
            ...
            Vector2 toBall = (ball.transform.position - transform.position).normalized;
            float dot = Vector2.Dot(transform.up, toBall);

            // Si el jugador está apuntando bastante cerca hacia la pelota (por ejemplo, más de 0.95)
            if (dot > 0.7f)
            {
                Jump();
            }
